...
from generateConfiguration import generate_configurations_Baseline_NoExit_RF
...
def run_all_configs(X_train, y_train, X_test, y_test, configs, start_factor=None):
    ...
    results_csv_path = f"Baseline/WithoutExit/RF/Results/{configs[0]['dataset_name']}_Dynamic_start_fs.csv"
    if os.path.exists(results_csv_path):
        os.remove(results_csv_path)
    
    runtime_csv_path = f"Baseline/WithoutExit/RF/Results/Summary/{configs[0]['dataset_name']}_runtime_per_config.csv"
...
if __name__ == '__main__':
    ...
    for dataset_name in NAMES:
        ...
        configs = generate_configurations_Baseline_NoExit_RF(dataset_name)
