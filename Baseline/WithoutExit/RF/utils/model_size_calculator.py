...
class ModelSizeCalculator:
    ...
    # Add inside ModelSizeCalculator class (model_size_calculator.py)

    def calculate_baseline_model_size(self, model):
        """
        Baseline (single RF/GB) -> return a dict that MATCHES the early-exit format
        so your existing CSV/runtime loggers work without changes.
        """
        single = self.calculate_single_model_size(model)
        return {
            'total_nodes': single['total_nodes'],
            'total_leaves': single['total_leaves'],
            'total_memory_kb': single['total_memory_kb'],
            'total_pickle_kb': single.get('pickle_size_kb', -1),
            'total_physical_trees': single.get('n_physical_trees', single.get('n_estimators', -1)),
            'num_exits': 1,
            'avg_nodes_per_exit': single['total_nodes'],
            'per_exit_details': [{
                'exit': 1,
                'n_estimators': single.get('n_estimators', -1),
                'n_physical_trees': single.get('n_physical_trees', -1),
                'is_gradient_boosting': single.get('is_gradient_boosting', False),
                'nodes': single['total_nodes'],
                'leaves': single['total_leaves'],
                'memory_kb': single['total_memory_kb'],
                'pickle_size_kb': single.get('pickle_size_kb', -1),
                'avg_nodes_per_tree': single.get('avg_nodes_per_tree', -1),
                'avg_depth': single.get('avg_depth', -1),
                'max_depth': single.get('max_depth', -1),
            }]
        }
