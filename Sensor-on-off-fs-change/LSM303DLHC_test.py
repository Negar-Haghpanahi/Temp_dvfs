...
try:
    # --- Phase init: ignore 5s ---
    print(time.time(), "Phase init: ignore 5s")
    init_sensor()
    set_odr_Acc(1)
#    set_odr_mag(0x00)
    start_ts = time.time()
    time.sleep(5)
    end_ts = time.time()
    log_phase_buffer("init", start_ts, end_ts)

    # --- Phase 1: 1 Hz ---
    current_phase = "1hz"
    print(time.time(), "Phase 1: 1 Hz normal mode for 5s")
    set_odr_Acc(1)
 #   set_odr_mag(0x00)
    start_ts = time.time()
    time.sleep(5)
    end_ts = time.time()
    log_phase_buffer(current_phase, start_ts, end_ts)

    # --- Phase 5: 100 Hz ---
    current_phase = "100hz"
    print(time.time(), "Phase 5: 100 Hz normal mode for 5s")
    set_odr_Acc(100)
  #  set_odr_mag(0x10)
    start_ts = time.time()
    time.sleep(5)
    ...
    # --- Phase 7: 400 Hz ---
    current_phase = "400hz"
    print(time.time(), "Phase 7: 400 Hz normal mode for 5s")
    set_odr_Acc(400)
   # set_odr_mag(0x18)
    start_ts = time.time()
    time.sleep(5)
    ...
    # --- Phase 8: 1344 Hz ---
    current_phase = "1344hz"
    print(time.time(), "Phase 8: 1344 Hz normal mode for 5s")
    set_odr_Acc(1344)
    #set_odr_mag(0x1C)
    start_ts = time.time()
    time.sleep(5)
    ...
    end_ts = time.time()
    log_phase_buffer(current_phase, start_ts, end_ts)
    # --- Save all phases to CSV at once ---
    PHASE_CSV = "phase_log.csv"
    with open(PHASE_CSV, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["phase", "start_time", "end_time"])
        writer.writerows(phase_buffer)

    print(time.time(), f"Done. {sample_count} total samples saved to {CSV_FILE}")
    print(f"Phase info saved to {PHASE_CSV}")

except KeyboardInterrupt:
    print(f"\nStopped early. {sample_count} samples saved.")