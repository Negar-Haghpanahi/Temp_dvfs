import numpy as np
from sklearn.ensemble import GradientBoostingClassifier   # <-- CHANGED
...
class DynamicEarlyExitRF:
    ...
    def predict_one(self, x_full_one, sample_id=-1, print_trace=False , start_factor=None):
        ...
        for k in range(K):
            ...
            # for k>=1 append ONLY the new segment (sp_{k-1} -> sp_k) using factor_next
            if k >= 1:
                ...
                new_seg_ds = data_downsampler(factor_next, new_seg).downsample()
            ...
            # factor_used: seg0 uses start_factor, later stages use factor_next (for appended segment)
            factor_used_here = start_factor if k == 0 else int(factor_next)
            ...
            if print_trace:
                if tau is not None:
                    print(f"[Sample {sample_id}] Stage {k+1} | sp={sp} | factor_used={stage_info['factor_used']} | H={H:.4f} | tau={tau:.4f} | exit={exit_now}")
                else:
                    print(f"[Sample {sample_id}] Stage {k+1} (LAST) | sp={sp} | factor_used={stage_info['factor_used']} | H={H:.4f} | forced exit=True")
            
            stage_info["margin"] = m
            stage_info["gamma"] = None if gamma is None else float(gamma)

            if (not exit_now) and (k < K - 1):
                next_factor = self.ctrl.choose_factor(H, tau ,self.num_classes, H_prev=H_prev)
                stage_info["continued_with_factor"] = int(next_factor)
                factor_next = int(next_factor)
                if print_trace:
                    print(f"   -> NOT exit, next segment factor={next_factor}")
            H_prev = H
